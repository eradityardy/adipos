...
class Products extends MY_Controller
{
    ...
    function get_products($store_id) {
        ...
        if ($this->Admin) {
            $this->datatables->select($this->db->dbprefix('products').".id as pid, ".$this->db->dbprefix('products').".image as image, ".$this->db->dbprefix('products').".code as code, ".$this->db->dbprefix('products').".name as pname, type, ".$this->db->dbprefix('categories').".name as cname, psq.quantity, (CASE WHEN psq.price > 0 THEN psq.price ELSE {$this->db->dbprefix('products')}.price END) as price, barcode_symbology", FALSE);
        } else {
            $this->datatables->select($this->db->dbprefix('products').".id as pid, ".$this->db->dbprefix('products').".image as image, ".$this->db->dbprefix('products').".code as code, ".$this->db->dbprefix('products').".name as pname, type, ".$this->db->dbprefix('categories').".name as cname, psq.quantity, (CASE WHEN psq.price > 0 THEN psq.price ELSE {$this->db->dbprefix('products')}.price END) as price, barcode_symbology", FALSE);
        }
    ...
    function print_barcodes() {
        ...
        foreach ($products as $pr) {
            ...
            $html .= '<td><h4>' . $this->product_barcode($pr->code, $pr->barcode_symbology, 60) . '<br>'.$this->Settings->currency_prefix. ' ' . $this->izi->formatMoney($pr->price) . '</td>';
            $r++;
        }
        $html .= '</tr></tbody>
        </table>';
    ...
    function print_labels() {
        ...
        $html = "";
        foreach ($products as $pr) {
            $html .= '<div class="text-center labels break-after">' . $this->product_barcode($pr->code, $pr->barcode_symbology, 25) . '<br>' .$this->Settings->currency_prefix. ' ' . $this->izi->formatMoney($pr->price) . '</div>';
        }
        $this->data['links'] = $pagination;
        $this->data['html'] = $html;
    ...
    function single_barcode($product_id = NULL) {
        ...
        if($product->quantity > 0) {
            for ($r = 1; $r <= $product->quantity; $r++) {
                ...
                $html .= '<td><h4>' . $this->product_barcode($product->code, $product->barcode_symbology, 60) . ' <br><span class="price">'.$this->Settings->currency_prefix. ' ' . $this->izi->formatMoney($product->price) . '</span></td>';
            }
        } else {
            for ($r = 1; $r <= 10; $r++) {
            if ($r != 1) {
    ...
    function single_label($product_id = NULL, $warehouse_id = NULL) {
        ...
        if($product->quantity > 0) {
            for ($r = 1; $r <= $product->quantity; $r++) {
                $html .= '<div class="text-center labels">' . $this->product_barcode($product->code, $product->barcode_symbology, 25) . ' <br><span class="price">'.$this->Settings->currency_prefix. ' ' . $this->izi->formatMoney($product->price) . '</span></div>';
            }
        } else {
            for ($r = 1; $r <= 10; $r++) {
                $html .= '<div class="text-center labels">' . $this->product_barcode($product->code, $product->barcode_symbology, 25) . ' <br><span class="price">'.$this->Settings->currency_prefix. ' ' . $this->izi->formatMoney($product->price) . '</span></div>';
    ...
    function add() {
        ...
        if ($this->form_validation->run() == true) {
            ...
            // $this->izi->print_arrays($data, $items);
        }
